    ...
    <div class="col-md-12">
        <div class="row">
            <div class="col-12" id="imageInput">
                @if(isset($banner->id))
                    @if($banner->image && !empty($banner->image['image_path']) && $banner->image['image_path'] != '/default/default_image.png')
                        <label>{{ __("Upload Mobile banner image") }} <small class="text-muted optional-text">({{ __("Optional - Leave empty to keep current image") }})</small></label>
                        <input type="file" accept="image/*" data-plugins="dropify" name="image" class="dropify" data-default-file="{{$banner->image['proxy_url'].'1900/500'.$banner->image['image_path']}}" data-has-existing-image="true" />
                        <small class="text-muted note-text">{{ __("Note: You can upload a new image to replace the current one, or leave empty to keep the existing image.") }}</small>
                    @else
                        <label>{{ __("Upload Mobile banner image") }} <span class="text-danger">*</span></label>
                        <input type="file" accept="image/*" data-plugins="dropify" name="image" class="dropify" required data-has-existing-image="false" />
                    @endif
                @else
                    <label>{{ __("Upload Mobile banner image") }} <span class="text-danger">*</span></label>
                    <input data-default-file="" type="file" data-plugins="dropify" name="image" accept="image/*" class="dropify" required data-has-existing-image="false" />
                @endif
                <label class="logo-size text-right w-100">{{ __("Logo Size") }} 1920x500</label>
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="form-group" id="languageInput">
                    {!! Form::label('language', __('Language'),['class' => 'control-label']) !!} <span class="text-danger">*</span>
                    <select class="selectize-select form-control" name="language_id" required>
                        <option value="">{{ __("Select Language") }}</option>
                        @foreach($languages as $lang)
                            <option value="{{$lang->language->id}}" {{($banner->language_id == $lang->language->id) ? 'selected' : ''}}>
                                {{$lang->language->name}} ({{$lang->language->nativeName}})
                                @if($lang->is_primary == 1) - {{ __("Primary") }} @endif
                            </option>
                        @endforeach
                    </select>
                    <span class="invalid-feedback" role="alert">
