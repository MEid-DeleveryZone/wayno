...
@section('content')
        ...
        <div class="row">
            <div class="col-8 d-flex align-items-center">
                ...
                <div class="site_link position-relative ml-3">
                    <a href="{{route('productDetail', $product->url_slug)}}" target="_blank"><span id="pwd_spn"
                            class="password-span">{{route('productDetail', $product->url_slug)}}</span></a>
            ...
            <div class="col-4 text-right" style="margin: auto;">
                <button type="button" class="btn btn-info waves-effect waves-light text-sm-right saveProduct">
                    {{ __("Submit") }}</button>
            </div>
        </div>
        <div class="row mb-2">
            ...
            <div class="row">
                        ...
                        <div class="row mb-2 row-spacing">
                            ...
                            <div class="col-md-3" style="cursor: not-allowed;">
                                ...
                                <input type="hidden" name="category_id"
                                    value="{{$product->category ? $product->category->cat->category_id : $product->category_id}}">
                            </div>
                        </div>
                    </div>
                    <div class="card-box ">
                            ...
                            <div class="col-4 p-2 mt-0" style="margin:auto; padding: 8px !important;">
                                <select class="selectize-select form-control" id="language_id" name="language_id">
                                    @foreach($languages as $lang)
                                        <option value="{{$lang->langId}}" {{ ($lang->is_primary == 1) ? 'selected' : ''}}>
                        ...
                        <div class="row mb-2">
                            ...
                            <div class="col-12 mb-2">
                                {!! Form::label('title', __('Dimensions'), ['class' => 'control-label']) !!}
                                {!! Form::textarea('dimensions', $product->primary ? $product->primary->dimensions : $product->dimensions, ['class' => 'form-control', 'id' => 'dimensions', 'placeholder' => 'Dimensions', 'rows' => '3']) !!}
                            </div>
                            {{-- <div class="col-6 mb-2">
                                {!! Form::label('title', __('SLA-Same Emirates'),['class' => 'control-label']) !!}
                                {!! Form::textarea('sla_same_emirates', $product->sla_same_emirates,
                                ['class'=>'form-control', 'id' => 'sla_same_emirates', 'placeholder' => 'SLA-Same Emirates',
                                'rows' => '3']) !!}
                            </div> --}}
                            @php
                                $useDistanceBasedSla = isset($configData) && (int) ($configData->use_distance_based_sla ?? 0) === 1;
                            @endphp
                            <div class="col-md-6 {{ $useDistanceBasedSla ? 'd-none' : '' }}" id="legacy-sla-same-wrap">
                                <div class="form-group mb-3" id="due_inInput">
                                    <label for="sla_same_emirates">SLA-Same Emirates</label>
                                    <div class="input-group mb-3">
                                        <div class="input-group-prepend">
                                            <input type="text" class="form-control" name="sla_same_emirates"
                                                id="sla_same_emirates" placeholder="Enter SLA In"
                                                value="{{$product->sla_same_emirates}}" {{ $useDistanceBasedSla ? 'disabled' : '' }}>
                                        </div>
                                        <select class="custom-select" id="same_emirate_frequency_select"
                                            name="same_emirate_frequency" {{ $useDistanceBasedSla ? 'disabled' : '' }}>
                                            <option value="" {{ !in_array($product->same_emirate_frequency, ['days', 'hours']) ? 'selected' : '' }}>Select</option>
                                            <option value="days" {{ $product->same_emirate_frequency == 'days' ? 'selected' : '' }}>Day(s)</option>
                                            <option value="hours" {{ $product->same_emirate_frequency == 'hours' ? 'selected' : '' }}>Hour(s)</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                            <div class="{{ $useDistanceBasedSla ? 'col-md-12' : 'col-md-6' }}">
                                <div class="form-group mb-3" id="due_inInput">
                                    <label for="due_in">SLA-Different Emirates</label>
                                    <div class="input-group mb-3">
                                        <div class="input-group-prepend">
                                            <input type="text" class="form-control" name="sla_diff_emirates"
                                                id="sla_diff_emirates" placeholder="Enter SLA In"
                                                value="{{$product->sla_diff_emirates}}">
                                        </div>
                                        <select class="custom-select" id="diff_emirate_frequency_select"
                                            name="diff_emirate_frequency">
                                            <option value="" {{ !in_array($product->diff_emirate_frequency, ['days', 'hours']) ? 'selected' : '' }}>Select</option>
                                            <option value="days" {{ $product->diff_emirate_frequency == 'days' ? 'selected' : '' }}>Day(s)</option>
                                            <option value="hours" {{ $product->diff_emirate_frequency == 'hours' ? 'selected' : '' }}>Hour(s)</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                            {{-- <div class="col-6 mb-2">
                                {!! Form::label('title', __('SLA-Different Emirates'),['class' => 'control-label']) !!}
                                {!! Form::textarea('sla_diff_emirates', $product->sla_diff_emirates,
                                ['class'=>'form-control', 'id' => 'sla_diff_emirates', 'placeholder' => 'SLA-Different
                                Emirates', 'rows' => '3']) !!}
                            </div> --}}
                            @php
                                $distanceSlaGroupId = $product->getAttributes()['distance_sla_group_id'] ?? null;
                                $distanceSlaGroupSelected = $distanceSlaGroupId ? \App\Models\DistanceSlaGroup::find($distanceSlaGroupId) : null;
                            @endphp
                            <div class="col-12 mb-2 {{ $useDistanceBasedSla ? '' : 'd-none' }}"
                                id="distance-sla-group-wrap">
                                <label class="control-label">{{ __('SLA-Same Emirates') }}</label>
                                <select name="distance_sla_group_id" id="distance_sla_group_id" class="form-control"
                                    style="width:100%" data-placeholder="{{ __('Search SLA Distance Group...') }}" {{ $useDistanceBasedSla ? '' : 'disabled' }}>
                                    <option value=""></option>
                                    @if($distanceSlaGroupSelected)
                                        <option value="{{ $distanceSlaGroupSelected->id }}" selected>
                                            {{ $distanceSlaGroupSelected->name }}@if($distanceSlaGroupSelected->is_default)
                                            ({{ __('Default') }})@endif</option>
                                    @endif
                                </select>
                                <small
                                    class="form-text text-muted">{{ __('Leave empty to use the system default SLA distance group.') }}</small>
                                @error('distance_sla_group_id')
                                    <div class="text-danger small">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-12 mb-2">
                                {!! Form::label('title', __('Weight Description'), ['class' => 'control-label']) !!}
                                {!! Form::text('weight_description', $product->primary ? $product->primary->weight_description : $product->weight_description, ['class' => 'form-control', 'id' => 'weight_description', 'placeholder' => 'Weight Description']) !!}
                            </div>
                            <div class="col-12 mb-2">
                            ...
                            @if($product->has_variant == 0)
                                <div class="row mb-2">
                                    ...
                                    {{-- <div class="col-4 mb-2">
                                        ...
                                        {!! Form::text('cost_price', $product->variant[0]->cost_price, ['class'=>'form-control',
                                        'id' => 'cost_price', 'placeholder' => '200', 'onkeypress' => 'return isNumberKey(event)'])
                                        !!}
                                    </div> --}}
                                </div>
                            @endif
                            <div class="row mb-2">
                                @if($product->category->categoryDetail->type_id != 8)
                                    <div class="col-sm-4">
                                        ...
                                        <input type="checkbox" bid="" id="has_inventory" data-plugin="switchery"
                                            name="has_inventory" class="chk_box" data-color="#43bee1" {{$product->has_inventory == 1 ? 'checked' : ''}}>
                                    </div>
                                @endif
                                    ...
                                    <div class="row">
                                        @if($product->category->categoryDetail->type_id != 8)
                                            ...
                                            <div class="col-sm-4">
                                                ...
                                                <br />
                                                <input type="checkbox" bid="" id="sell_stock_out" data-plugin="switchery"
                                                    name="sell_stock_out" class="chk_box" data-color="#43bee1"
                                                    @if($product->sell_when_out_of_stock == 1) checked @endif>
                                            </div>
                                        @endif
                                        @if($configData->need_dispacher_home_other_service == 1 && $product->category->categoryDetail->type_id == 8)
                                            {{-- <div class="col-sm-4">
                                                {!! Form::label('title', 'Need Price From Dispatcher',['class' => 'control-label'])
                                                !!} <br />
                                                <input type="checkbox" bid="" id="need_price_from_dispatcher"
                                                    data-plugin="switchery" name="need_price_from_dispatcher" class="chk_box"
                                                    data-color="#43bee1" @if($product->need_price_from_dispatcher == 1) checked
                                                @endif>
                                            </div> --}}
                                        @endif
                            ...
                            <div class="row mb-2 bg-light">
                                ...
                                @if(!empty($productVariants))
                                    <div class="col-4 p-2 mt-0 text-right" style="margin:auto; ">
                                        <button type="button" class="btn btn-info makeVariantRow">
                                            {{ __("Make Variant Sets") }}</button>
                                    </div>
                                @endif
                            </div>
                                            ...
                                            <div class="col-sm-9">
                                                @foreach($var->option as $key => $opt)
                                                    <div class="checkbox checkbox-success form-check-inline pr-3">
                                                        <input type="checkbox" name="variant{{$var->id}}" class="intpCheck"
                                                            opt="{{$opt->id . ';' . $opt->title}}" varId="{{$var->id . ';' . $var->title}}"
                                                            id="opt_vid_{{$opt->id}}" @if(in_array($opt->id, $existOptions)) checked
                                                            @endif>
                                                        ...
                                                        $vsets .= $vs->title . ', ';
                                                                            ...
                                                                            <tr id="tr_{{$varnt->id}}">
                                                                                <td>
                                                                                    <div class="image-upload">
                                                                                        <label class="file-input uploadImages" for="{{$varnt->id}}">
                                                                                            <img src="{{$mediaPath}}" width="30" height="30"
                                                                                                for="{{$varnt->id}}" />
                                                                                        </label>
                                                                                    </div>
                                                                                ...
                                                                                <td>
                                                                                    ...
                                                                                    <input type="hidden" class="exist_sets"
                                                                                        value="{{$existSet[(count($existSet) - 1)]}}">
                                                                                    <input type="text" name="variant_titles[]" value="{{$varnt->title}}">
                                                                                </td>
                                                                                <td>{{rtrim($vsets, ', ')}}</td>
                                                                                <td>
                                                                                    <input type="text" style="width: 70px;" name="variant_price[]"
                                                                                        value="{{$varnt->price}}" onkeypress="return isNumberKey(event)">
                                                                                </td>
                                                                                <td>
                                                                                    <input type="text" style="width: 100px;" name="variant_compare_price[]"
                                                                                        value="{{$varnt->compare_at_price}}"
                                                                                        onkeypress="return isNumberKey(event)">
                                                                                </td>
                                                                                <td>
                                                                                    <input type="text" style="width: 70px;" name="variant_cost_price[]"
                                                                                        value="{{$varnt->cost_price}}" onkeypress="return isNumberKey(event)">
                                                                                </td>
                                                                                <td class="check_inventory">
                                                                                    <input type="text" style="width: 70px;" name="variant_quantity[]"
                                                                                        value="{{$varnt->quantity}}" onkeypress="return isNumberKey(event)">
                                                                                </td>
                                                                                <td>
                                                                                    <a href="javascript:void(0);" data-varient_id="{{$varnt->id}}"
                                                                                        class="action-icon deleteExistRow">
                ...
                <div class="col-lg-5">
                    ...
                    <div class="card-box">
                        ...
                        <div class="row mb-2">
                            @if($client_preference_detail->business_type != 'taxi' && Auth::user()->is_superadmin == 1)
                                <div class="col-md-6 d-flex justify-content-between mb-2">
                                    {!! Form::label('title', __('New'), ['class' => 'control-label']) !!}
                                    <input type="checkbox" id="is_new" data-plugin="switchery" name="is_new" class="chk_box"
                                        data-color="#43bee1" @if($product->is_new == 1) checked @endif>
                                </div>
                                <div class="col-md-6 d-flex justify-content-between mb-2">
                                    ...
                                    <input type="checkbox" id="is_featured" data-plugin="switchery" name="is_featured"
                                        class="chk_box" data-color="#43bee1" @if($product->is_featured == 1) checked @endif>
                                </div>
                            @endif
                            @if($configData->need_delivery_service == 1 && $product->category->categoryDetail->type_id != 7)
                                <div class="col-md-6 d-flex justify-content-between mb-2">
                                    ...
                                    <input type="checkbox" id="last_mile" data-plugin="switchery" name="last_mile"
                                        class="chk_box" data-color="#43bee1" @if($product->Requires_last_mile == 1) checked
                                        @endif>
                                </div>
                            @endif
                            @if($configData->pharmacy_check == 1)
                                <div class="col-md-6 d-flex justify-content-between mb-2">
                                    ...
                                    <input type="checkbox" bid="" id="pharmacy_check" data-plugin="switchery"
                                        name="pharmacy_check" class="chk_box" data-color="#43bee1"
                                        @if($product->pharmacy_check == 1) checked @endif>
                                </div>
                            @endif
                            @if($configData->enquire_mode == 1)
                                <div class="col-md-6 d-flex justify-content-between mb-2">
                                    {!! Form::label('title', __('Inquiry Only'), ['class' => 'control-label']) !!}
                                    <input type="checkbox" bid="" id="inquiry_only" data-plugin="switchery" name="inquiry_only"
                                        class="chk_box" data-color="#43bee1" @if($product->inquiry_only == 1) checked @endif>
                                </div>
                            @endif


                            @if($configData->need_dispacher_ride == 1 && ($product->category->categoryDetail->type_id == 7 || $product->category->categoryDetail->type_id == 9 || $product->category->categoryDetail->type_id == 10))
                                <div class="col-md-6 d-flex justify-content-between mb-2">
                                    ...
                                    <select class="selectize-select1 form-control" name="tags" required>
                                        ...
                                        @if($agent_dispatcher_tags != null && count($agent_dispatcher_tags))
                                            @foreach($agent_dispatcher_tags as $key => $tags)
                                                @if($tags['name'] != "")
                                                    <option value="{{ $tags['name'] }}" @if($product->tags == $tags['name'])
                                                    selected="selected" @endif>{{ ucfirst($tags['name']) }}</option>
                                                @endif
                                            @endforeach
                                        @endif
                                    </select>
                            ...
                            @if($configData->need_dispacher_home_other_service == 1 && $product->category->categoryDetail->type_id == 8)
                                <div class="col-md-6 d-flex justify-content-between mb-2">
                                    ...
                                    <select class="selectize-select1 form-control" name="mode_of_service" required>
                                        <option value="instant" @if($product->mode_of_service == 'instant') selected="selected"
                                        @endif>{{ __('Instant') }}</option>
                                        <option value="schedule" @if($product->mode_of_service == 'schedule') selected="selected"
                                        @endif>{{ __('Schedule') }}</option>
                                    </select>
                                </div>
                            @endif
                            ...
                            <div class="col-md-6 d-flex justify-content-between mb-2">
                                ...
                                <input type="checkbox" bid="" id="returnable" data-plugin="switchery" name="returnable"
                                    class="chk_box" data-color="#43bee1" @if($product->returnable == 1) checked @endif>
                            </div>
                        </div>
                            ...
                            <div class="col-md-6 mb-2">
                                ...
                                <select class="form-control " id="typeSelectBox" name="tax_category">
                                    ...
                                    @foreach($taxCate as $cate)
                                        <option value="{{$cate->id}}" @if($product->variant[0]->tax_category_id == $cate->id)
                                        selected @endif>{{$cate->title}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="row mb-2">
                            ...
                            <div class="col-sm-4">
                                <input type="checkbox" bid="" id="is_physical" data-plugin="switchery" name="is_physical"
                                    class="chk_box" data-color="#43bee1" @if($product->is_physical == 1) checked @endif>
                            </div>
                        </div>
                        <div class="row mb-2 physicalDiv"
                            style="{{ ($product->is_physical == 1) ? '' : 'display: none;' }}">
                            <div class="col-sm-6">
                        ...
                        <div class="row mb-2 physicalDiv" style="{{ ($product->is_physical == 1) ? '' : 'display:none;' }}">
                            ...
                            <div class="col-sm-3 mb-2">
                                <input type="checkbox" id="requiredShipping" data-plugin="switchery" name="require_ship"
                                    class="chk_box" data-color="#43bee1" @if($product->requires_shipping == 1) checked @endif>
                            </div>
                            <div class="col-sm-6"></div>
                            <div class="col-sm-6 shippingDiv"
                                style="{{($product->requires_shipping == 1) ? '' : 'display:none;' }}">
                                ...
                                <select class="form-control selectizeInput" id="country_origin_id" name="country_origin_id">
                                    @foreach($countries as $coun)
                                        <option value="{{$coun->id}}" @if($product->country_origin_id == $coun->id) selected
                                        @endif>{{$coun->name}}</option>
                                    @endforeach
                                </select>
                                    ...
                                    if (isset($media->image) && is_array($media->image->path)) {
                                        $mediaPath = $media->image->path['proxy_url'] . '300/300' . $media->image->path['image_path'];
                                                        ...
                                                        <div class="product-action">
                                                            <a href="{{route('product.deleteImg', [$product->id, $media->image->id])}}"
                                                                class="btn btn-danger btn-xs waves-effect waves-light"
                                                                onclick="return confirm('Are you sure? You want to delete the image.')"><i
                                                                    class="mdi mdi-close" {{$media->image}}></i></a>
                                                        </div>
                                                        <div class="bg-light">
                            ...
                            <div class="row">
                                @if($configData->celebrity_check == 1)
                                    <div class="col-md-6 mb-2">
                                        ...
                                        <select class="form-control select2-multiple" name="celebrities[]" data-toggle="select2"
                                            multiple="multiple" placeholder="Select celebrity...">
                                            @foreach($celebrities as $cel)
                                                <option value="{{$cel->id}}" @if(in_array($cel->id, $celeb_ids)) selected @endif>
                                                    {{$cel->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                ...
                                <div class="col-md-6 mb-2">
                                    ...
                                    <select class="form-control select2-multiple" name="addon_sets[]" data-toggle="select2"
                                        multiple="multiple" placeholder="Select addon...">
                                        @foreach($addons as $set)
                                            <option value="{{$set->id}}" @if(in_array($set->id, $addOn_ids)) selected @endif>
                                                {{$set->title}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                @if($product->category->categoryDetail->type_id != 8)
                                    <div class="col-md-6 mb-2">
                                        ...
                                        <select class="form-control select2-multiple" name="up_cell[]" data-toggle="select2"
                                            multiple="multiple" placeholder="Select gear...">
                                            @foreach($otherProducts as $otherProduct)
                                                <option value="{{$otherProduct->id}}" @if(in_array($otherProduct->id, $upSell_ids))
                                                selected @endif>{{$otherProduct->primary->title}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="col-md-6 mb-2">
                                        ...
                                        <select class="form-control select2-multiple" name="cross_cell[]" data-toggle="select2"
                                            multiple="multiple" placeholder="Select gear...">
                                            @foreach($otherProducts as $otherProduct)
                                                <option value="{{$otherProduct->id}}" @if(in_array($otherProduct->id, $crossSell_ids))
                                                selected @endif>{{$otherProduct->primary->title}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-6 mb-2">
                                        ...
                                        <select class="form-control select2-multiple" name="releted_product[]" data-toggle="select2"
                                            multiple="multiple" placeholder="Select gear...">
                                            @foreach($otherProducts as $otherProduct)
                                                <option value="{{$otherProduct->id}}" @if(in_array($otherProduct->id, $related_ids))
                                                selected @endif>{{$otherProduct->primary->title}}</option>
                                            @endforeach
                                        </select>
                                    </div>
    ...
    <div id="upload-media" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"
        style="display: none;">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button"
                        class="btn btn-info waves-effect waves-light selectVaiantImages">{{ __("Select") }}</button>
                </div>
            </div>
        </div>
    </div>
...
@section('script')
            ...
            $.ajax({
                ...
                success: function (resp) {
                    if (resp) {
                        ...
                        $('#dimensions').val(resp.data.dimensions);
                        $('#weight_description').val(resp.data.weight_description);
    ...
    <script>
        ...
        $('#distance_sla_group_id').select2({
            placeholder: "{{ __('Search SLA Distance Group...') }}",
            allowClear: true,
            ajax: {
                url: "{{ route('distance-sla-groups.search') }}",
                dataType: 'json',
                delay: 250,
        ...
        $('.product_form').on('submit', function () {
            $('#legacy-sla-same-wrap').find('input, select').prop('disabled', false);
            $('#distance_sla_group_id').prop('disabled', false);
        });
    </script>
@endsection